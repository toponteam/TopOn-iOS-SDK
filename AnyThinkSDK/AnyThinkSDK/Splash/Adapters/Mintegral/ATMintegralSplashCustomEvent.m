...
- (void)splashADPreloadSuccess:(id<ATMTGSplashAD>)splashAD {
    ...
    if (_timeRemaining > [[NSDate date] timeIntervalSinceDate:_loadStartingDate]) {
        ...
        [self handleLoadingFailure:[NSError errorWithDomain:@"com.anythink.MintegralSplashLoading" code:0 userInfo:@{NSLocalizedDescriptionKey:@"AnyThinkSDK has failed to load splash", NSLocalizedFailureReasonErrorKey:@"It took too long for MTGSDK to load splash"}]];
    }
}
...
- (void)splashADPreloadFail:(id<ATMTGSplashAD>)splashAD error:(NSError *)error {
    ...
    [self handleLoadingFailure:error != nil ? error : [NSError errorWithDomain:@"com.anythink.MintegralSplashLoading" code:0 userInfo:@{NSLocalizedDescriptionKey:@"AnyThinkSDK has failed to load splash", NSLocalizedFailureReasonErrorKey:@"MTGSDK has failed to load splash"}]];
}
...
- (void)splashADShowSuccess:(id<ATMTGSplashAD>)splashAD { [ATLogger logMessage:@"MintegralSplash::splashADShowSuccess" type:ATLogTypeExternal]; }
...
-(NSDictionary*)delegateExtra {
    ...
    extra[kATADDelegateExtraNetworkPlacementIDKey] = self.ad.unitGroup.content[@"unitid"];
    return extra;
}
@end
