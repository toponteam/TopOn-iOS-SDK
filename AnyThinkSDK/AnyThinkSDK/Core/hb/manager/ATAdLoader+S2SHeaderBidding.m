...
+(void) sendS2SBidRequestWithPlacementModel:(ATPlacementModel*)placementModel headerBiddingUnitGroups:(NSArray<ATUnitGroupModel*>*)headerBiddingUnitGroups requestID:(NSString*)requestID extra:(NSDictionary *) extra completion:(void(^)(NSDictionary<NSString*, ATBidInfo*>*bidInfos, NSDictionary<NSString*, NSError*>*errors))completion {
    ...
    NSString *hbListStr = [[[ATAdLoader headerBiddingListParameterWithUnitGroups:headerBiddingUnitGroups extra:extra] jsonString_anythink] stringByBase64Encoding_anythink];
    ...
    //send s2s header bidding request and parse response for waterfall
    [[ATNetworkingManager sharedManager] sendHTTPRequestToAddress:[ATAppSettingManager sharedManager].adxSetting.bidHttpAddress HTTPMethod:ATNetworkingHTTPMethodPOST parameters:para gzip:NO completion:^(NSData * _Nonnull data, NSURLResponse * _Nullable response, NSError * _Nullable error) {
        ...
        if ([responseObject isKindOfClass:[NSDictionary class]] && [responseObject[@"data"] isKindOfClass:[NSArray class]]) {
            if (completion != nil) {
                //hb response data parse support mtg&adx
                ...
                [headerBiddingUnitGroups enumerateObjectsUsingBlock:^(ATUnitGroupModel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
                    NSString *remoteUnitId = @"";
                    if([obj.adapterClass respondsToSelector:@selector(adsourceRemoteKeyWithContent:unitGroupModel:)]){
                        remoteUnitId = [obj.adapterClass adsourceRemoteKeyWithContent:obj.content unitGroupModel:obj];
                    }
                    ugMap[[NSString stringWithFormat:@"%@_%d", remoteUnitId, obj.networkFirmID]] = obj; }];
                ...
                [dataArr enumerateObjectsUsingBlock:^(NSDictionary * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
                    if ([obj[@"is_success"] boolValue]) {
                        NSString *mapKey = [NSString stringWithFormat:@"%@_%d", obj[@"unit_id"], [obj[@"nw_firm_id"] integerValue]];
                        ATBidInfo *bidInfo = [[ATBidInfo alloc] initWithDictionary:obj placementID:placementModel.placementID unitGroupUnitID:ugMap[mapKey].unitID expirationInterval:ugMap[mapKey].bidTokenTime];
...
+(NSArray<NSDictionary*>*)headerBiddingListParameterWithUnitGroups:(NSArray<ATUnitGroupModel*>*)unitGroups extra:(NSDictionary *) extra {
    NSMutableArray<NSDictionary*>* headerBiddingList = [NSMutableArray<NSDictionary*> array];
    [unitGroups enumerateObjectsUsingBlock:^(ATUnitGroupModel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) { [headerBiddingList addObject:[obj.adapterClass respondsToSelector:@selector(headerBiddingParametersWithUnitGroupModel:extra:)] ? [obj.adapterClass headerBiddingParametersWithUnitGroupModel:obj extra:extra] : @{}]; }];
