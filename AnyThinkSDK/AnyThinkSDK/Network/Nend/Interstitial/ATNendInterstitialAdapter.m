...
-(void) loadADWithInfo:(NSDictionary*)serverInfo localInfo:(NSDictionary*)localInfo completion:(void (^)(NSArray<NSDictionary *> *, NSError *))completion {
    if (NSClassFromString(kInterstitialClassName) != nil && NSClassFromString(kInterstitialVideoClassName) != nil && NSClassFromString(kFullscreenInterstitialClassName) != nil && NSClassFromString(kFullscreenInterstitialLoaderClassName) != nil) {
        dispatch_async(dispatch_get_main_queue(), ^{
            self->_customEvent = [[ATNendInterstitialCustomEvent alloc] initWithInfo:serverInfo localInfo:localInfo];
            ...
            if ([serverInfo[@"is_video"] integerValue] == 0) {
                ...
                self->_interstitialVideo = [[NSClassFromString(kInterstitialVideoClassName) alloc] initWithSpotId:serverInfo[@"spot_id"] apiKey:serverInfo[@"api_key"]];
                self->_interstitialVideo.delegate = self->_customEvent;
                if ([extra isKindOfClass:[NSDictionary class]]) {
                    if ([extra[kATInterstitialExtraMediationNameKey] isKindOfClass:[NSString class]]) { self->_interstitialVideo.mediationName = extra[kATInterstitialExtraMediationNameKey]; }
                    if ([extra[kATInterstitialExtraUserIDKey] isKindOfClass:[NSString class]]) { self->_interstitialVideo.userId = extra[kATInterstitialExtraUserIDKey]; }
                    if ([extra[kATInterstitialExtraUserFeatureKey] isKindOfClass:[NSClassFromString(@"NADUserFeature") class]]) { self->_interstitialVideo.userFeature = extra[kATInterstitialExtraUserFeatureKey]; }
                    if ([extra[kATInterstitialExtraLocationEnabledFlagKey] respondsToSelector:@selector(boolValue)]) { self->_interstitialVideo.isLocationEnabled = [extra[kATInterstitialExtraLocationEnabledFlagKey] boolValue]; }
                    if ([extra[kATInterstitialExtraMuteStartPlayingFlagKey] respondsToSelector:@selector(boolValue)]) { self->_interstitialVideo.isMuteStartPlaying = [extra[kATInterstitialExtraMuteStartPlayingFlagKey] boolValue]; }
                    if ([extra[kATInterstitialExtraFallbackFullboardBackgroundColorKey] isKindOfClass:[UIColor class]]) { self->_interstitialVideo.fallbackFullboardBackgroundColor = extra[kATInterstitialExtraFallbackFullboardBackgroundColorKey]; }
                }
                [self->_interstitialVideo addFallbackFullboardWithSpotId:serverInfo[@"spot_id"] apiKey:serverInfo[@"api_key"]];
                [self->_interstitialVideo loadAd];
            } else if ([serverInfo[@"is_video"] integerValue] == 2) {
                self->_fullBoardLoader = [[NSClassFromString(kFullscreenInterstitialLoaderClassName) alloc] initWithSpotId:serverInfo[@"spot_id"] apiKey:serverInfo[@"api_key"]];
                __weak typeof(self) weakSelf = self;
                [self->_fullBoardLoader loadAdWithCompletionHandler:^(id<ATNADFullBoard> ad, NSInteger error) {
