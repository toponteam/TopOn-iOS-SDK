...
// MARK:- ATInmobiRewardedVideoBiddingDelegate
@interface ATInmobiRewardedVideoBiddingDelegate : NSObject<ATIMInterstitialDelegate>

@property(nonatomic, readonly) NSMutableDictionary<NSString *, ATInmobiBiddingRequest *> *requests;

@end

@implementation ATInmobiRewardedVideoBiddingDelegate

+ (instancetype)sharedInstance {
    static ATInmobiRewardedVideoBiddingDelegate *sharedDelegate = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        sharedDelegate = [[ATInmobiRewardedVideoBiddingDelegate alloc] init];
    });
            ...
            if (currentValue == 0) {//not inited
                ...
                [NSClassFromString(@"IMSdk") initWithAccountID:serverInfo[@"app_id"] andCompletionHandler:^(NSError *error) {
                    if (error == nil) {
                        ...
                        completion(nil, error != nil ? error : [NSError errorWithDomain:@"com.anythink.InmobiRewardedVideoLoading" code:0 userInfo:@{NSLocalizedDescriptionKey:ATSDKAdLoadFailedErrorMsg, NSLocalizedFailureReasonErrorKey:@"IMSDK has failed to initialize"}]);
                    }
                }];
                return 1;
...
-(void) loadADUsingInfo:(NSDictionary*)serverInfo localInfo:(NSDictionary *)localInfo completion:(void (^)(NSArray<NSDictionary*> *assets, NSError *error))completion {
    ...
    if (bidInfo) {
        ...
        [[ATInmobiRewardedVideoBiddingDelegate sharedInstance].requests removeObjectForKey:serverInfo[kUnitIDKey]];
...
+(void) bidRequestWithPlacementModel:(ATPlacementModel*)placementModel unitGroupModel:(ATUnitGroupModel*)unitGroupModel info:(NSDictionary*)info completion:(void(^)(ATBidInfo *bidInfo, NSError *error))completion {
    ...
    ATInmobiRewardedVideoCustomEvent *customEvent = [[ATInmobiRewardedVideoCustomEvent alloc] initWithInfo:info localInfo:nil];
    ...
    [[ATInmobiRewardedVideoBiddingDelegate sharedInstance] startWithRequestItem:request];
}
