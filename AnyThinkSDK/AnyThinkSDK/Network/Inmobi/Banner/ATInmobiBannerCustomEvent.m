...
- (void)banner:(id<ATIMBanner>)banner gotSignals:(NSData*)signals {
    [ATLogger logMessage:@"InmobiBanner::banner:gotSignals:" type:ATLogTypeExternal];
}

- (void)banner:(id<ATIMBanner>)banner failedToGetSignalsWithError:(id)status {
    [ATLogger logMessage:[NSString stringWithFormat:@"InmobiBanner::banner:failedToGetSignalsWithError:%@", status] type:ATLogTypeExternal];
}

-(void)banner:(id<ATIMBanner>)banner didReceiveWithMetaInfo:(id)info {
    [ATLogger logMessage:[NSString stringWithFormat:@"InmobiBanner::banner:didReceiveWithMetaInfo:"] type:ATLogTypeExternal];
}

-(void)banner:(id<ATIMBanner>)banner didFailToReceiveWithError:(id)error {
    [ATLogger logMessage:[NSString stringWithFormat:@"InmobiBanner::banner:didFailToReceiveWithError:%@", error] type:ATLogTypeExternal];
}
...
-(void)bannerDidFinishLoading:(id<ATIMBanner>)banner {
    ...
    [self trackBannerAdLoaded:banner adExtra:@{kAdAssetsPriceKey: _price ? _price : @"0", kAdAssetsBidIDKey : _bidID ? _bidID : @""}];
}
