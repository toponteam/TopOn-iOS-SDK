//
//  ATGoogleAdManagerNativeCustomEvent.m
...
@interface ATGoogleAdManagerNativeCustomEvent()
...
@end
@implementation ATGoogleAdManagerNativeCustomEvent
-(instancetype) init {
    self = [super init];
    if (self != nil) {
...
- (void)adLoader:(id<ATDFPAdLoader>)adLoader didFailToReceiveAdWithError:(NSError*)error {
    [ATLogger logMessage:[NSString stringWithFormat:@"ATGoogleAdManagerCustomEvent::adLoader:didFailToReceiveAdWithError:%@", error] type:ATLogTypeExternal];
...
- (void)adLoaderDidFinishLoading:(id<ATDFPAdLoader>)adLoader {
    [ATLogger logMessage:@"ATGoogleAdManagerCustomEvent::adLoaderDidFinishLoading:" type:ATLogTypeExternal];
    dispatch_async(dispatch_get_main_queue(), ^{
        if (!_finished) {
            self.requestCompletionBlock([_offers count] > 0 ? _offers : nil, [_offers count] > 0 ? nil : [NSError errorWithDomain:ATADLoadingErrorDomain code:ATADLoadingErrorCodeADOfferLoadingFailed userInfo:@{NSLocalizedDescriptionKey:kATSDKFailedToLoadNativeADMsg, NSLocalizedFailureReasonErrorKey:@"GoogleAdManager sdk did not return any valid response."}]);
            _finished = YES;
        }
    });
}
...
- (void)adLoader:(id<ATDFPAdLoader>)adLoader didReceiveUnifiedNativeAd:(id<ATGADUnifiedNativeAd>)nativeAd {
    [ATLogger logMessage:@"ATGoogleAdManagerCustomEvent::adLoader:didReceiveUnifiedNativeAd:" type:ATLogTypeExternal];
    NSMutableDictionary *assets = [NSMutableDictionary dictionaryWithObjectsAndKeys:self, kAdAssetsCustomEventKey, nativeAd, kAdAssetsCustomObjectKey, self.unitID, kNativeADAssetsUnitIDKey, nil];
...
- (void)nativeAdWillLeaveApplication:(id<ATGADUnifiedNativeAd>)nativeAd {
    [ATLogger logMessage:@"ATGoogleAdManagerCustomEvent:nativeAdWillLeaveApplication:" type:ATLogTypeExternal];
    if (!_clicked) [ATLogger logMessage:@"GoogleAdManager ad will take you out of the app, but click delegate not supported" type:ATLogTypeInternal];
}

- (void)nativeAdDidRecordImpression:(id<ATGADUnifiedNativeAd>)nativeAd {
    [ATLogger logMessage:@"ATGoogleAdManagerCustomEvent:nativeAdDidRecordImpression:" type:ATLogTypeExternal];
    [self trackNativeAdImpression];
}

- (void)nativeAdDidRecordClick:(id<ATGADUnifiedNativeAd>)nativeAd {
    [ATLogger logMessage:@"ATGoogleAdManagerCustomEvent::nativeAdDidRecordClick:" type:ATLogTypeExternal];
    [self trackNativeAdClick];
    _clicked = YES;
}

- (void)videoControllerDidPlayVideo:(id<ATGADVideoController>)videoController {
    [ATLogger logMessage:@"ATGoogleAdManagerCustomEvent::videoControllerDidPlayVideo:" type:ATLogTypeExternal];
    [self trackNativeAdVideoStart];
}

- (void)videoControllerDidEndVideoPlayback:(id<ATGADVideoController>)videoController {
    [ATLogger logMessage:@"ATGoogleAdManagerCustomEvent::videoControllerDidEndVideoPlayback:" type:ATLogTypeExternal];
