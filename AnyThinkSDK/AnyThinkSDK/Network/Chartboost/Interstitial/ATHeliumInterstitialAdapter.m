...
@property(nonatomic, readonly) NSMutableDictionary<NSString*, ATHeliumInterstitialCustomEvent *> *showingCustomEventStorage;
@end
...
-(instancetype) init {
    ...
    if (self != nil) {
        ...
        _showingCustomEventStorage = [NSMutableDictionary<NSString*, ATHeliumInterstitialCustomEvent *> dictionary];

    }
    return self;
}

-(void) setHeliumShowingCustomEvent:(ATHeliumInterstitialCustomEvent *)customEvent placementName:(NSString*)placementName {
    __weak typeof(self) weakSelf = self;
    [_heliumInterstitialAdStorageAccessor writeWithBlock:^{
        [weakSelf.showingCustomEventStorage setValue:customEvent forKey:placementName];
    }];
}

-(ATHeliumInterstitialCustomEvent *) getHeliumShowingCustomEventWithPlacementName:(NSString*)placementName {
    __weak typeof(self) weakSelf = self;

    return [_heliumInterstitialAdStorageAccessor readWithBlock:^id{
        return [weakSelf.showingCustomEventStorage objectForKey:placementName];
    }];
}
...
-(void) initHeliumSDKWithInfo:(NSDictionary*)info unitGroupModel:(ATUnitGroupModel *) unitGroupModel{
    void(^blk)(void) = ^{
        ...
        if ([ATAppSettingManager sharedManager].complyWithCCPA) {
            [(id<ATHeliumSdk>)[NSClassFromString(@"HeliumSdk") sharedHelium] setCCPAConsent:NO];
        }
        if ([ATAppSettingManager sharedManager].complyWithCOPPA) {
            [(id<ATHeliumSdk>)[NSClassFromString(@"HeliumSdk") sharedHelium] setSubjectToCoppa:YES];
        }
    };
...
- (void)heliumDidStartWithError:(id<HeliumError>)error  {
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:kATHeliumInitNotification object:nil userInfo:error != nil ? @{@"key_error":error} : @{}];
}
...
//    NSMutableDictionary* cacheDictionary = [self getHeliumInterstitialAdWithPlacementName:placementName];
//    ATHeliumInterstitialCustomEvent* customevent = cacheDictionary[@"at_customevent"];
    ATHeliumInterstitialCustomEvent* customevent = [self getHeliumShowingCustomEventWithPlacementName:placementName];
    if (error == nil) {
...
+(void) showInterstitial:(ATInterstitial*)interstitial inViewController:(UIViewController*)viewController delegate:(id<ATInterstitialDelegate>)delegate {
    ...
    [[ATHeliumInterstitialSharedDelegate sharedDelegate] setHeliumShowingCustomEvent:(ATHeliumInterstitialCustomEvent *)interstitial.customEvent placementName:interstitial.unitID];
    ...
    if (![[ATAPI sharedInstance] initFlagForNetwork:kNetworkNameHelium]) {
        ...
        if(cacheDictionary != nil){
            ...
            if([_customEvent.interstitialAd readyToShow]){
                [_customEvent trackInterstitialAdLoaded:_customEvent.interstitialAd adExtra:@{kAdAssetsPriceKey:bidInfo.price, kAdAssetsBidIDKey:bidInfo.bidId}];
                cacheDictionary[@"at_is_loaded"] = @YES;
            } else {
