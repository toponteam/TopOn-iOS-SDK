...
@class ATGCDAsyncReadPacket;
@class ATGCDAsyncWritePacket;
@class ATGCDAsyncSocketPreBuffer;
@protocol ATGCDAsyncSocketDelegate;

NS_ASSUME_NONNULL_BEGIN

extern NSString *const ATGCDAsyncSocketException;
extern NSString *const ATGCDAsyncSocketErrorDomain;

extern NSString *const ATGCDAsyncSocketQueueName;
extern NSString *const ATGCDAsyncSocketThreadName;

extern NSString *const ATGCDAsyncSocketManuallyEvaluateTrust;
#if TARGET_OS_IPHONE
extern NSString *const ATGCDAsyncSocketUseCFStreamForTLS;
#endif
...
extern NSString *const ATGCDAsyncSocketSSLPeerID;
extern NSString *const ATGCDAsyncSocketSSLProtocolVersionMin;
extern NSString *const ATGCDAsyncSocketSSLProtocolVersionMax;
extern NSString *const ATGCDAsyncSocketSSLSessionOptionFalseStart;
extern NSString *const ATGCDAsyncSocketSSLSessionOptionSendOneByteRecord;
extern NSString *const ATGCDAsyncSocketSSLCipherSuites;
extern NSString *const ATGCDAsyncSocketSSLALPN;
#if !TARGET_OS_IPHONE
extern NSString *const ATGCDAsyncSocketSSLDiffieHellmanParameters;
#endif

#define GCDAsyncSocketLoggingContext 65535


typedef NS_ERROR_ENUM(ATGCDAsyncSocketErrorDomain, GCDAsyncSocketError) {
...
@interface ATGCDAsyncSocket : NSObject

/**
 * ATGCDAsyncSocket uses the standard delegate paradigm,
 ...
 * If you pass NULL, ATGCDAsyncSocket will automatically create it's own socket queue.
...
- (instancetype)initWithDelegate:(nullable id<ATGCDAsyncSocketDelegate>)aDelegate delegateQueue:(nullable dispatch_queue_t)dq;
- (instancetype)initWithDelegate:(nullable id<ATGCDAsyncSocketDelegate>)aDelegate delegateQueue:(nullable dispatch_queue_t)dq socketQueue:(nullable dispatch_queue_t)sq NS_DESIGNATED_INITIALIZER;

/**
 * Create ATGCDAsyncSocket from already connect BSD socket file descriptor
**/
...
+ (nullable instancetype)socketFromConnectedSocketFD:(int)socketFD delegate:(nullable id<ATGCDAsyncSocketDelegate>)aDelegate delegateQueue:(nullable dispatch_queue_t)dq socketQueue:(nullable dispatch_queue_t)sq error:(NSError **)error;

#pragma mark Configuration

@property (atomic, weak, readwrite, nullable) id<ATGCDAsyncSocketDelegate> delegate;
...
- (void)getDelegate:(id<ATGCDAsyncSocketDelegate> __nullable * __nullable)delegatePtr delegateQueue:(dispatch_queue_t __nullable * __nullable)delegateQueuePtr;
- (void)setDelegate:(nullable id<ATGCDAsyncSocketDelegate>)delegate delegateQueue:(nullable dispatch_queue_t)delegateQueue;
...
**/
- (void)synchronouslySetDelegate:(nullable id<ATGCDAsyncSocketDelegate>)delegate;
- (void)synchronouslySetDelegateQueue:(nullable dispatch_queue_t)delegateQueue;
- (void)synchronouslySetDelegate:(nullable id<ATGCDAsyncSocketDelegate>)delegate delegateQueue:(nullable dispatch_queue_t)delegateQueue;

/**
 * By default, both IPv4 and IPv6 are enabled.
 * 
 * For accepting incoming connections, this means ATGCDAsyncSocket automatically supports both protocols,
 * and can simulataneously accept incoming connections on either protocol.
 * 
 * For outgoing connections, this means ATGCDAsyncSocket can connect to remote hosts running either protocol.
 * If a DNS lookup returns only IPv4 results, ATGCDAsyncSocket will automatically use IPv4.
 * If a DNS lookup returns only IPv6 results, ATGCDAsyncSocket will automatically use IPv6.
 ...
 * When a connection is accepted, a new instance of ATGCDAsyncSocket will be spawned to handle it,
 ...
 * 
 * Please note the recommended way of releasing a ATGCDAsyncSocket instance (e.g. in a dealloc method)
 ...
 * This is due to the fact that ATGCDAsyncSocket does NOT copy the data. It simply retains it.
 ...
 * 
 * - ATGCDAsyncSocketManuallyEvaluateTrust
 ...
 *     So ATGCDAsyncSocket will invoke the delegate method socket:shouldTrustPeer: passing the SecTrustRef.
 *
 ...
 *
 * - ATGCDAsyncSocketUseCFStreamForTLS (iOS only)
 *     The value must be of type NSNumber, encapsulating a BOOL value.
 *     By default ATGCDAsyncSocket will use the SecureTransport layer to perform encryption.
 ...
 *     
 *     However, if you absolutely must, you can instruct ATGCDAsyncSocket to use the old-fashioned encryption
 *     technique by going through the CFStream instead. So instead of using SecureTransport, ATGCDAsyncSocket
 ...
 *     
 *     Thus all the other keys in the given dictionary will be ignored by ATGCDAsyncSocket,
 ...
 *
 * - ATGCDAsyncSocketSSLProtocolVersionMin
 * - ATGCDAsyncSocketSSLProtocolVersionMax
 ...
 * 
 * - ATGCDAsyncSocketSSLSessionOptionFalseStart
 *     The value must be of type NSNumber, encapsulating a BOOL value.
 *     See Apple's documentation for kSSLSessionOptionFalseStart.
 * 
 * - ATGCDAsyncSocketSSLSessionOptionSendOneByteRecord
 *     The value must be of type NSNumber, encapsulating a BOOL value.
 *     See Apple's documentation for kSSLSessionOptionSendOneByteRecord.
 * 
 * - ATGCDAsyncSocketSSLCipherSuites
 ...
 *
 * - ATGCDAsyncSocketSSLDiffieHellmanParameters (Mac OS X only)
 *     The value must be of type NSData.
 ...
 *     You MUST use manual trust evaluation instead (see ATGCDAsyncSocketManuallyEvaluateTrust).
 ...
 *     You MUST use ATGCDAsyncSocketSSLProtocolVersionMin & ATGCDAsyncSocketSSLProtocolVersionMin instead.
...
/**
 * ATGCDAsyncSocket maintains thread safety by using an internal serial dispatch_queue.
 ...
 * - (dispatch_queue_t)newSocketQueueForConnectionFromAddress:(NSData *)address onSocket:(ATGCDAsyncSocket *)sock
 * {
 ...
 *     return socketQueue;
 * }
 * - (void)socket:(ATGCDAsyncSocket *)sock didAcceptNewSocket:(ATGCDAsyncSocket *)newSocket
 * {
...
@protocol ATGCDAsyncSocketDelegate <NSObject>
@optional
...
**/
- (nullable dispatch_queue_t)newSocketQueueForConnectionFromAddress:(NSData *)address onSocket:(ATGCDAsyncSocket *)sock;
 ...
 * The host parameter will be an IP address, not a DNS name.
 **/
- (void)socket:(ATGCDAsyncSocket *)sock didConnectToUrl:(NSURL *)url;
...
**/
- (void)socket:(ATGCDAsyncSocket *)sock didReadPartialDataOfLength:(NSUInteger)partialLength tag:(long)tag;
 ...
 * It may be used for things such as updating progress bars.
**/
- (void)socket:(ATGCDAsyncSocket *)sock didWritePartialDataOfLength:(NSUInteger)partialLength tag:(long)tag;
...
**/
- (NSTimeInterval)socket:(ATGCDAsyncSocket *)sock shouldTimeoutReadWithTag:(long)tag
...
**/
- (NSTimeInterval)socket:(ATGCDAsyncSocket *)sock shouldTimeoutWriteWithTag:(long)tag
                                                                  elapsed:(NSTimeInterval)elapsed
                                                                bytesDone:(NSUInteger)length;
...
**/
- (void)socketDidCloseReadStream:(ATGCDAsyncSocket *)sock;
...
**/
- (void)socket:(ATGCDAsyncSocket *)sock didReceiveTrust:(SecTrustRef)trust
