...
-(void) loadADWithInfo:(NSDictionary*)serverInfo localInfo:(NSDictionary*)localInfo completion:(void (^)(NSArray<NSDictionary *> *, NSError *))completion {
    if (NSClassFromString(@"NADNative") != nil && NSClassFromString(@"NADNativeClient") != nil && NSClassFromString(@"NADNativeVideo") != nil && NSClassFromString(@"NADNativeVideoLoader") != nil) {
        ...
        if ([serverInfo[@"is_video"] boolValue]) {//Video
            _loaders = [NSMutableArray<id<ATNADNativeVideoLoader>> arrayWithCapacity:[serverInfo[@"request_num"] integerValue]];
                ...
                if (loader != nil) {
                    ...
                    [loader loadAdWithCompletionHandler:^(id<ATNADNativeVideo> ad, NSError *error) {
                        dispatch_async(dispatch_get_main_queue(), ^{
                            [weakSelf handleAdRequestResultWithAd:ad error:error index:i loadingInfo:serverInfo];
                        });
                    }];
                }
            }
        } else {//Non-video
            _clients = [NSMutableArray<id<ATNADNativeClient>> arrayWithCapacity:[serverInfo[@"request_num"] integerValue]];
            for (NSInteger i = 0; i < [serverInfo[@"request_num"] integerValue]; i++) {
